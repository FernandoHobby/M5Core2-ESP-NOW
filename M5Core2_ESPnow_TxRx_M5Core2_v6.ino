...
void setup() {
  ...
  Serial.println("ESP-NOW ejemplo comunicacion bidireccional ROL MASTER");
  M5.Lcd.println("ESP-NOW ejemplo comunicacion bidireccional master<>esclavo");
...
void loop() {
  ...
  if (!esp_now_is_peer_exist(M5StampC3_STA_MAC)) {
    iniciarEsclavo(M5StampC3_STA_MAC);
  } else {
    texto = "Envio desde MASTER M5SCore2 a ESCLAVO M5StampC3_STA_MAC = " + String(millis());
    enviarEsclavo(M5StampC3_STA_MAC, id, contador++, texto);
    delay(250);
  }

  if (!esp_now_is_peer_exist(ESP32_STA_MAC)) {
    iniciarEsclavo(ESP32_STA_MAC);
  } else {
    texto = "Envio desde MASTER M5SCore2 a ESCLAVO ESP32_STA_MAC = " + String(millis());
    enviarEsclavo(ESP32_STA_MAC, id, contador++, texto);
    delay(250);
  }
...
void enviarEsclavo(uint8_t macEnviar[], int id, int contador, String texto) {
  ...
  switch (result) {
    case ESP_OK:
      ...
      M5.Lcd.print("<-Ok enviado a ESCLAVO = ");
      M5.Lcd.println(macConvStr(macEnviar));
      break;
    case ESP_ERR_ESPNOW_NOT_INIT:
      Serial.println("ESP-NOW no iniciado");
      break;
...
void OnMensajeEnviado(const uint8_t *macEnviado, esp_now_send_status_t status) {
  ...
  M5.Lcd.print(status == ESP_NOW_SEND_SUCCESS ? "<-Ok recibido por ESCLAVO = " : "<-Error NO recibe esclavo = ");
...
void OnMensajeRecibido(const uint8_t *macRecibido, const uint8_t *recMensaje, int len) {
  ...
  Serial.print("->Ok recibido mensaje de ESCLAVO = ");
  ...
  if (debug) {
    Serial.printf("Bytes mensajeRecibido = %d\n",len);
