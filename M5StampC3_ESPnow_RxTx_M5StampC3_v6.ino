...
void setup() {
  ...
  Serial.println("ESP-NOW ejemplo comunicacion bidireccional ROL ESCLAVO");
...
void loop() {
  ...
  if (!esp_now_is_peer_exist(M5Core2_STA_MAC)) {
    ...
    String txt = "Envio desde ESCLAVO M5StampC3 a MASTER M5Core2 = " + String(millis());
    enviarEsclavo(M5Core2_STA_MAC, id, contador++, txt);
    delay(250);
  }
...
void OnMensajeRecibido(const uint8_t *macRecibido, const uint8_t *recMensaje, int len) {
  ...
  Serial.print("->Ok recibido de MASTER = ");
