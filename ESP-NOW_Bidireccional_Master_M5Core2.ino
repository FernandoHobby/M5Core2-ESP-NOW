...
void setup() {
  ...
  Serial.println("ESP-NOW ejemplo comunicacion bidireccional M5Core2_STA_MAC - ROL MASTER");
  M5.Lcd.println("ESP-NOW ejemplo comunicacion bidireccional M5Core2_STA_MAC - ROL MASTER");
...
void loop() {
  ...
  if (millis() > tiempo + periodoEnviar){
    enviarEsclavo(M5StampC3_STA_MAC, id, contador++, "Envio desde MASTER M5SCore2 a ESCLAVO M5StampC3_STA_MAC");  
    tiempo = millis();
  }
  ...
  if (recibidoECO) {
    if (debug) {
      ...
      Serial.print("mensajeRecibido.texto = "); Serial.println(mensajeRecibido.texto);
      if (M5.Lcd.getCursorY()>200){
        M5.Lcd.clear();
        M5.Lcd.setCursor(0,20);
      }
      M5.Lcd.printf("Recibido ECO contador = %d\n",mensajeRecibido.contador);
    }
    recibidoECO = false;
  }
